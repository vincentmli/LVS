				  ...
				  __u32 count, __u32 maxdiff)
{
	...
	IP_VS_DBG(6, "syn_cookie check cookie=%u\n", cookie);
	IP_VS_DBG(6, "syn_cookie check vs syn seq=%u\n", sseq);
	IP_VS_DBG(6, "syn_cookie check count=%u\n", count);

	/* Cookie is now reduced to (count * 2^24) ^ (hash % 2^24) */
	diff = (count - (cookie >> COOKIEBITS)) & ((__u32) - 1 >> COOKIEBITS);
	IP_VS_DBG(6, "syn_cookie check diff=%u\n", diff);
	if (diff >= maxdiff)
		return (__u32)-1;

	IP_VS_DBG(6, "syn_cookie check excuted second hash");
	return (cookie -
